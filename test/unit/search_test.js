...
describe("Search Service", function() {
    ...
    it('should report a warning when response is not from all shards', function() {
        $httpBackend.expectPOST('/api/search/?query=bug&from=0&size=10&status=&tags=').respond(partiallySuccessfulEmtpySearchResponse);
        result = service.query("bug");
        $httpBackend.flush();
        expect(result.inProgress).toBe(false);
        expect(result.errorMessage).toBe("");
    ...
    it('should report an error when search API returns an error', function() {
        $httpBackend.expectPOST('/api/search/?query=bug&from=0&size=10&status=&tags=').respond(500, 'dial tcp 127.0.0.1:9200: connection refused');
        service.query("bug");
        $httpBackend.flush();
        expect(result.inProgress).toBe(false);
